...
class GETR(nn.Module):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.rgb_input_proj = nn.Conv2d(1536, 256, kernel_size=1)
        self.fusion = nn.Conv2d(256, 256, 1)
        self.mask_head = MaskHeadSmallConv(512, [1536, 768, 384, 192], 256)
        #self.transformer = build_transformer()
        self.lcfi_3 = LCFI(1536, lcfiType=2)
        self.lcfi_2 = LCFI(768, lcfiType=2)
        self.lcfi_1 = LCFI(384, lcfiType=1)
        self.lcfi_0 = LCFI(192, lcfiType=1)
        self.lcfi_channel_3 = nn.Conv2d(384, 256, kernel_size=1)
        self.lcfi_channel_2 = nn.Conv2d(192, 256, kernel_size=1)
        self.lcfi_channel_1 = nn.Conv2d(96, 256, kernel_size=1)
        self.lcfi_channel_0 = nn.Conv2d(48, 256, kernel_size=1)
        ...
        nn.init.kaiming_uniform_(self.lcfi_channel_3.weight, a=1)
        nn.init.kaiming_uniform_(self.lcfi_channel_2.weight, a=1)
        nn.init.kaiming_uniform_(self.lcfi_channel_1.weight, a=1)
        nn.init.kaiming_uniform_(self.lcfi_channel_0.weight, a=1)
    ...
    def forward(self, inputs):
        ...
        rgb_features_lcfi_256channel_3 = self.lcfi_channel_3(rgb_features_lcfi_3)
        rgb_features_lcfi_256channel_2 = self.lcfi_channel_2(rgb_features_lcfi_2)
        rgb_features_lcfi_256channel_1 = self.lcfi_channel_1(rgb_features_lcfi_1)
        rgb_features_lcfi_256channel_0 = self.lcfi_channel_0(rgb_features_lcfi_0)
        
        fusion_memory_3 = self.fusion_module_3(torch.cat(
            (F.interpolate(rgb_features_lcfi_256channel_2, size=inputs[3].shape[-2:], mode="bilinear"), 
             F.interpolate(rgb_features_lcfi_256channel_1, size=inputs[3].shape[-2:], mode="bilinear"), 
             F.interpolate(rgb_features_lcfi_256channel_0, size=inputs[3].shape[-2:], mode="bilinear")),1))
        fusion_memory_2 = self.fusion_module_2(torch.cat(
            (F.interpolate(rgb_features_lcfi_256channel_3, size=inputs[2].shape[-2:], mode="bilinear"), 
             F.interpolate(rgb_features_lcfi_256channel_1, size=inputs[2].shape[-2:], mode="bilinear"), 
             F.interpolate(rgb_features_lcfi_256channel_0, size=inputs[2].shape[-2:], mode="bilinear")),1))
        fusion_memory_1 = self.fusion_module_1(torch.cat(
            (F.interpolate(rgb_features_lcfi_256channel_3, size=inputs[1].shape[-2:], mode="bilinear"), 
             F.interpolate(rgb_features_lcfi_256channel_2, size=inputs[1].shape[-2:], mode="bilinear"), 
             F.interpolate(rgb_features_lcfi_256channel_0, size=inputs[1].shape[-2:], mode="bilinear")),1))
        fusion_memory_0 = self.fusion_module_0(torch.cat(
            (F.interpolate(rgb_features_lcfi_256channel_3, size=inputs[0].shape[-2:], mode="bilinear"), 
             F.interpolate(rgb_features_lcfi_256channel_2, size=inputs[0].shape[-2:], mode="bilinear"), 
             F.interpolate(rgb_features_lcfi_256channel_1, size=inputs[0].shape[-2:], mode="bilinear")),1))
...
class LCFI(nn.Module):
    def __init__(self, input_channels, dr1=1, dr2=2, dr3=3, dr4=4, lcfiType = 1):
...
class MaskHeadSmallConv(nn.Module):
    ...
    def __init__(self, dim, fpn_dims, context_dim):
        ...
        self.rgb_adapter1 = nn.Sequential(nn.Conv2d(1536, inter_dims[1], 1),
                                          nn.BatchNorm2d(inter_dims[1]),
                                          nn.ReLU(inplace=True))
        self.rgb_adapter2 = nn.Sequential(nn.Conv2d(768, inter_dims[1], 1),
                                          nn.BatchNorm2d(inter_dims[1]),
                                          nn.ReLU(inplace=True))
        self.rgb_adapter3 = nn.Sequential(nn.Conv2d(384, inter_dims[2], 1),
                                          nn.BatchNorm2d(inter_dims[2]),
                                          nn.ReLU(inplace=True))
        self.rgb_adapter4 = nn.Sequential(nn.Conv2d(192, inter_dims[3], 1),
    ...
    def forward(self, fusion_memory, rgb_fpns, rgb_lcfi, gate_flag):
        ...
        x = self.inference_module1(self.pa_module1(x, rgb_cur_fpn, rgb_lcfi[0], gate_flag))  # 1/32

        rgb_cur_fpn = self.rgb_adapter2(rgb_fpns[1])

        x = F.interpolate(x, size=rgb_cur_fpn.shape[-2:], mode="bilinear")
        x = self.inference_module2(self.pa_module2(x, rgb_cur_fpn, rgb_lcfi[1], gate_flag))  # 1/16

        rgb_cur_fpn = self.rgb_adapter3(rgb_fpns[2])

        x = F.interpolate(x, size=rgb_cur_fpn.shape[-2:], mode="bilinear")
        x = self.inference_module3(self.pa_module3(x, rgb_cur_fpn, rgb_lcfi[2], gate_flag))  # 1/8

        rgb_cur_fpn = self.rgb_adapter4(rgb_fpns[3])

        x = F.interpolate(x, size=rgb_cur_fpn.shape[-2:], mode="bilinear")
        x = self.inference_module4(self.pa_module4(x, rgb_cur_fpn, rgb_lcfi[3], gate_flag))  # 1/4
...
class PixelAttention(nn.Module):
    ...
    def forward(self, x, rgb, lcfi, gate_flag):
        ...
        rx = x + rgb * mask1
        mask2 = self.mask_conv2(torch.cat([x, lcfi], 1))
