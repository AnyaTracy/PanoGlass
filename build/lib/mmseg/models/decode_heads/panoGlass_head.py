...
class MyHead(BaseDecodeHead):
    ...
    def forward(self, inputs):
        ...
        # print('output[0][3]',output[0][3])
        # print('output[0][4]',output[0][4])
        # print('output[0][5]',output[0][5])
        # print('output[0][6]',output[0][6])
        # print('output[0][7]',output[0][7])
        # print('output[0][8]',output[0][8])
        # print('output[0][9]',output[0][9])
        # print('output[0][10]',output[0][10])
        # print('output[0][11]',output[0][11])
        return output
...
class MaskHeadSmallConv(nn.Module):
    ...
    def forward(self, fusion_memory, rgb_fpns, rgb_hdcn, gate_flag):

        x = fusion_memory

        rgb_cur_fpn = self.rgb_adapter1(rgb_fpns[0])

        x = self.inference_module1(self.pa_module1(x, rgb_cur_fpn, rgb_hdcn[0], gate_flag))  # 1/32

        rgb_cur_fpn = self.rgb_adapter2(rgb_fpns[1])

        x = F.interpolate(x, size=rgb_cur_fpn.shape[-2:], mode="bilinear")
        x = self.inference_module2(self.pa_module2(x, rgb_cur_fpn, rgb_hdcn[1], gate_flag))  # 1/16

        rgb_cur_fpn = self.rgb_adapter3(rgb_fpns[2])

        x = F.interpolate(x, size=rgb_cur_fpn.shape[-2:], mode="bilinear")
        x = self.inference_module3(self.pa_module3(x, rgb_cur_fpn, rgb_hdcn[2], gate_flag))  # 1/8

        rgb_cur_fpn = self.rgb_adapter4(rgb_fpns[3])

        x = F.interpolate(x, size=rgb_cur_fpn.shape[-2:], mode="bilinear")
        x = self.inference_module4(self.pa_module4(x, rgb_cur_fpn, rgb_hdcn[3], gate_flag))  # 1/4
...
class PixelAttention(nn.Module):
    ...
    def forward(self, x, rgb, hdcn, gate_flag):
