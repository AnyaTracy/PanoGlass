...
class CustomDataset(Dataset):
    ...
    def pre_eval(self, preds, orig_preds, indices):
        ...
        if not isinstance(preds, list):
            preds = [preds]
        if not isinstance(orig_preds, list):
            orig_preds = [orig_preds]

        pre_eval_results = []
        mae = 0
        for pred, orig_pred, index in zip(preds, orig_preds, indices):
            #print(orig_pred)
            seg_map = self.get_gt_seg_map_by_idx(index)
            #print(seg_map)
            #print('before',np.sum(seg_map))
            #seg_map = seg_map==1
            #print('later',np.sum(seg_map))
            #print(orig_pred.shape)
            mae += np.mean(np.abs(np.float32(orig_pred[0][1].cpu())-np.float32(seg_map)))
            j=0
